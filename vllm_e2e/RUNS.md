...
| 40 | branch untangle + decode-1k reproduction gate | 2414307 | Discovered the run-31-39 flashinfer tree (MoEWeightPack union + warn-once DSL check) had run as UNCOMMITTED working-tree changes on fi-moe_ep-vllm-integration@58761040; committed today as moe_ep-weight-pack-union (d80deefd/6e2cf21e), which is now the checked-out benchmark tree. Reproduction: exact run-38 headline decode cell (decode:128:256 @1024 seqs, capture 64..4096, knob_cache_dsv4_dec2k, nvfp4 ckpt), 3 rounds, fresh node ptyche0252 | median **33717.7 total tok/s vs run-38's 33921.7 (-0.6%)**, rounds 33859/33718/33683 (<0.6% spread) — headline decode config REPRODUCES on the committed tree; run-37's dec1k-knob variant measured 33678, i.e. today's number sits exactly in the established band. Numbers-intact gate PASSED for the branch surgery | results/requick_fi_dec2k.json |

| 41 | native-4.5.2 venv revalidation (FRESH rebuild, no 4.6.1 chain) | 2427445 (2026-07-22) | FRESH=1 setup_container.sh on the new default stack: vllm 0.25.1's own nvidia-cutlass-dsl==4.5.2 pin kept (no quack/tvm-ffi/tilelang upgrades, no ThrMma patch), fi branch 4_5_2-perf-fix (MR!27 mainloop WAR). New hard DSL guards active (setup guard + bench EXPECT_DSL=4.5.2). Run-37/40 decode-1k cell, native + fi_nvfp4 dec2k knobs, 3 rounds | Both cells green on dsl=4.5.2: native **32085.7** total / 21390.5 output tok/s; fi_nvfp4 **34262.8** total / 22841.9 output = **1.068x native**, rounds 34254/34263/34282 (<0.1% spread) — inside/above the 4.6.1-stack band (33718-33922 total, 1.067-1.074x). Native-4.5.2 e2e REVALIDATED; the 4.6.1 compat chain is officially unnecessary. Benign teardown-only tracebacks in worker.shutdown() (tilelang 0.1.9 libcudart_stub lacks cudaDeviceReset — post-round, engine-exit path only) | results/requick452_native_dec.json, requick452_fi_dec2k.json |

| 42 | native-4.5.2 prefill-8k headline cell — 1.18x REPRODUCES | 2427572 (2026-07-22) | Completes the 4.5.2 revalidation pair (run 41 = decode). Exact run-34 recipe: prefill:1024:1 @256 prompts, MAX_BATCHED_TOKENS=8192, graphs w/ sparse capture 64,256,1024,2048,4096,8192, GPU_MEM_UTIL=0.95, fi on knob_cache_dsv4_8k, 5 rounds, EXPECT_DSL=4.5.2 guard green both cells | native **45581.6** / fi_nvfp4 **53622.9** total tok/s = **1.176x** (run-34 4.6.1 reference: 45701/53962 = 1.181x; both cells within 0.7%). HEADLINE PAIR CONFIRMED ON NATIVE 4.5.2: prefill 1.18x / decode 1.07x — the published numbers stand on the pin-free stack | results/requick452_native_pre8k.json, requick452_fi_pre8k.json |

| 43 | Flash EP4 12-cell sweep (from-scratch reproduction) | 2337172 (2026-07-25) | vllm-pr branch, fresh scratch_runbook checkout, DSL 4.5.2, B200 | 12/12 cells, rc=0. fi_cutedsl 1.151x pre8k / 1.051x dec1k / 1.080x lc100k / 1.137x ctx32k; fi_dg 1.016-1.022x. **dec1k rows superseded by run 49** (unpinned CAPTURE_SIZES, see run 47) | scratch_runbook/.../results/sweep_*.json, vllm_pr_sweep_2337172.log |

| 44 | Flash EP8 12-cell sweep | 2337204 (2026-07-25) | as run 43, TP8/EP8 | 12/12, rc=0. fi_cutedsl 1.199x/1.057x/1.111x/1.178x. fi_dg 1.02x everywhere EXCEPT dec1k at **0.620x** with TTFT 7.6s vs 1.9s — root-caused in run 47, corrected in run 49 | scratch_runbook/.../results/sweep_ep8_*.json |

| 45 | V4-Pro EP8 12-cell sweep (first Pro e2e) | 2337438 (2026-07-25) | V4-Pro mx hf-0366e4e_orig + pinned NVFP4 9e7e88ee, TP8/EP8, knob_cache_pro_ep8 | **11/12** — dec1k/fi_cutedsl died at engine init ("No available memory for the cache blocks"). fi_cutedsl 1.309x pre8k / 1.232x lc100k / 1.293x ctx32k (stronger than Flash); fi_dg 1.02x except dec1k **0.444x**. Both failures share one cause (run 47) | scratch_runbook/.../results/sweep_pro_*.json |

| 46 | GSM8K cross-checkpoint gate, Flash + Pro, both checkpoints | 2337476 (2026-07-25) | native/fi_dg on mx, fi_cutedsl on the NVFP4 cast, --model passed explicitly per cell | **Caught that the gate had been disarmed.** The 03:42 run (2337127) exported MODEL, which resolve_model ranks above the per-backend default, so its "fi_nvfp4" row scored the mx checkpoint — the NVFP4 weights behind every fi_cutedsl number had never been gated. Re-armed: Flash 0.960/0.960/**0.970**, Pro 0.880/0.880/**0.890**, cross-checkpoint delta +0.010 on both models = casts validated. Pro is below the 0.93 gate on all three backends (not a moe_ep issue) — see run 50 | results/gsm8k2_*.json |

| 47 | V4-Pro dec1k root cause | 2337473 (2026-07-25) | dec1k at GPU_MEM_UTIL 0.93 and at the 0.92 default, full engine logs retained (the sweep's cell() greps them away) | **vLLM's CUDA-graph memory profiler over-reserves ~48 GiB/GPU for both flashinfer backends against a real capture cost of ~6 GiB** — the same ~6 GiB it estimates correctly for native. It comes out of the KV cache: native 48.91 GiB / 95,979 tok / 1024 seqs resident / 13268 tok/s; fi_dg 7.28 GiB / 14,286 tok / **189** seqs (835 waiting, KV 98.5%) / 5969 tok/s; fi_cutedsl 0.07 GiB / engine will not start. Small batches are why fi_dg's ITL looked *better* (42.6 vs 94.4 ms) while throughput collapsed. Trigger: dec1k was the only cell setting MAX_CAPTURE without pinning CAPTURE_SIZES | scratch_runbook/.../results/diag_pro_dec1k_*.log |

| 48 | V4-Pro dec1k fix — CAPTURE_SIZES pinned | 2337487 (2026-07-25) | CAPTURE_SIZES=256,1024,2048,4096, nothing else changed | Recovers both failures. fi_dg 5893 -> 13143 tok/s (**2.23x**, now 1.021x native); fi_cutedsl boots and runs at 15327 (**1.191x**) — the 0.93 fallback never triggered, so it was the capture ladder, not headroom. All three sustain ~1240 concurrency. Native costs 3% to padding (13272 -> 12872), so pre/post-fix dec1k numbers are NOT comparable. Fix landed in all three job_vllm_pr_runbook_sweep*.sh and RUNBOOK_REPRO §5c | results/dec1kfix_*.json |

| 49 | Flash dec1k fix at EP4 + EP8 | 2337549 (2026-07-25) | pinned cell at both EP sizes, plus the unpinned cell at each size as a diagnostic | All 8 cells ran; the job's own summary step died on a shell-quoting bug (double quotes inside the python heredoc terminate the outer `bash -lc`), numbers recomputed from the JSONs. **EP8 corrected**: native 30724, fi_dg 31517 (**1.026x**, was 0.620x), fi_cutedsl 32695 (1.064x). **EP4 corrected**: native 30506, fi_dg 31094 (1.019x), fi_cutedsl 32226 (1.056x). **Why EP4 was never bitten**: its phantom reservation was ~11.6 GiB (0.92->0.8555) vs Pro EP8's ~48 GiB, and Flash EP4 ran at 24.8% KV usage -- so much headroom the inflated estimate never bit. Unpinned fi_dg held 1280 concurrent there vs 520 at EP8 and 189 on Pro. The bug was latent at EP4, not absent, so the published EP4 numbers were sound. Pinning costs ~3% uniformly — corrects the run-43/44 dec1k rows and tests why EP4 survived the unpinned ladder (1.022x) while EP8 did not (0.620x); a pure weights-vs-KV argument predicts the opposite, so the phantom reservation likely scales with world size | flash_dec1k_fix_2337549.log |

| 50 | V4-Pro GSM8K token budget | 2337550 (2026-07-25) | Pro native at max_tokens 512/1024/2048 + Flash native at 512, eval now records `truncated` | **Truncation hypothesis REFUTED.** Pro 512 -> 0.8800 (15/200 truncated), 1024 -> 0.8750 (14), 2048 -> 0.8750 (13): raising the cap does not recover accuracy, and 13 completions still never terminate at 2048 (runaway generation, not a tight budget). Flash truncates 3/200 at the same 512 cap, so Pro does run long 5x more often -- it is just not what costs it the 8 points. V4-Pro genuinely scores ~0.88 on this eval; --min-acc 0.93 is calibrated for Flash and is the wrong threshold for Pro. The cross-checkpoint delta (+0.010, run 46) is the number that gates perf claims, and it passes — decides whether Pro's 0.880 is the 512-token cap truncating a longer reasoner (a truncated chain still ends in a number, so it scores as a confident wrong answer, matching the observed 24 wrong / 0 unparseable) or a real deficit that makes --min-acc 0.93 the wrong threshold for Pro | results/gsm8ktok_*.json |
