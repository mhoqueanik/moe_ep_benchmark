...
# flashinfer moe_ep (FI_MOE_EP=1): swap the FusedMoE factory's default
# RoutedExperts for the fi-backed variant before any MoE layer is built.
from .fi_experts import fi_v32_enabled as _fi_v32_enabled

if _fi_v32_enabled():
    from .fi_experts import install as _fi_install
...
class DeepseekV32DecoderLayer(torch.nn.Module):
    def __init__(
        ...
        if (
            ...
            if _fi_v32_enabled():
                # forward_monolithic needs the runner's router for expert
                # selection; RoutedExperts has no back-reference by design.
                self.mlp.experts.routed_experts._fi_router = self.mlp.experts.router
        else:
